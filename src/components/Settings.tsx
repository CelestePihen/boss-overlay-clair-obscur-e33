...
interface Props {
  ...
  currentPath: string
  onConfigChange?: (config: { allowManualEditAutoDetected?: boolean; allowBossEditing?: boolean }) => void
}
...
function Settings({ onSavePathChange, currentPath, onConfigChange }: Props) {
  ...
  return (
    <div className="settings">
      ...
      <div className="setting-group" style={{ marginTop: '20px', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '20px' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
          ...
          />
          <span>Autoriser la modification manuelle des boss détectés automatiquement</span>
        </label>
        <p style={{ fontSize: '12px', color: '#95a5a6', marginTop: '8px', marginLeft: '28px' }}>
          Par défaut, seuls les boss ajoutés manuellement peuvent être modifiés. Activez cette option pour pouvoir corriger les boss détectés automatiquement depuis la sauvegarde.
        </p>
        ...
        <p style={{ fontSize: '12px', color: '#95a5a6', marginTop: '8px', marginLeft: '28px' }}>
          Affiche le bouton ✏️ pour modifier le nom, la zone et la catégorie des boss.
        </p>
      </div>
      ...
      {currentPath && (
        <div className="setting-group" style={{ marginTop: '20px', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '20px' }}>
          ...
          >
            {isClearing ? '⏳ Suppression...' : '🗑️ Réinitialiser les modifications manuelles'}
          </button>
      ...
      <div className="info">
        ...
        <p>L'overlay se mettra à jour automatiquement quand vous tuez un boss</p>
      </div>
    </div>
  )
}
